...
def upload_image():
    ...
    operation_selection = request.form['operation_selection']
...
def water_effect(decode_array_to_img):
    water_effect = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.15)
...
def invert_image(decode_array_to_img):
    ...
    status, output_img = cv2.imencode('.PNG', inverted_image)

    return output_img

def hdr(decode_array_to_img):
    hdr_image = cv2.detailEnhance(decode_array_to_img, sigma_s=60, sigma_r=0.15)
    status, output_img = cv2.imencode('.PNG', hdr_image)
